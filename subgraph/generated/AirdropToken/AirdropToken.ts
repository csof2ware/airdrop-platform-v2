...
export class AirdropToken extends ethereum.SmartContract {
  static bind(address: Address): AirdropToken {
    return new AirdropToken("AirdropToken", address);
  }

  EARLY_ADOPTER(): BigInt {
    let result = super.call("EARLY_ADOPTER", "EARLY_ADOPTER():(uint256)", []);

    return result[0].toBigInt();
  }

  try_EARLY_ADOPTER(): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "EARLY_ADOPTER",
      "EARLY_ADOPTER():(uint256)",
      [],
    );
    if (result.reverted) {
  ...
  VIP(): BigInt {
    let result = super.call("VIP", "VIP():(uint256)", []);

    return result[0].toBigInt();
  }

  try_VIP(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("VIP", "VIP():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
